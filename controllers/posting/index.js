...
class PostingController {
    ...
    async getAllPosting(req){
        ...
        if (!page){
            ...
            throw { statusCode: 400, msg: `Unsatisfied: Invalid page type`}
    ...
    async getOnePosting(req){
        ...
        if (!posting){
            throw { statusCode: 500, msg: `Failed: Posting can be got due to internal server error`}
    ...
    async createOnePosting(req){
        ...
        if (!posting){
            throw { statusCode: 500, msg: `Failed: Posting can not be created due to internal server error`}
        } else if (posting === "unauthorized"){
            throw { statusCode: 403, msg: `Forbidden: User does not have access to the required posting`}
    ...
    async changeOnePosting(req){
        ...
        if (!posting){
            ...
            throw {statusCode: 404, msg: `Posting not found`}
        }
    ...
    async deleteOnePosting(req){
        const postingId = getAndValidateObjectId(req, "_id")
        if (!postingId){
            throw { statusCode: 400, msg: `Unsatisfied: Missing posting id`}
        }
        let posting = await postingService.deleteOnePosting(req.session.user, postingId)
        if (!posting){
            throw { statusCode: 500, msg: `Failed: Posting can not be delted due to internal server error`}
        } else if (posting === "unauthorized"){
            throw { statusCode: 403, msg: `Unauthorized: User does not have access to the required posting`}
        } else if (posting === "not found"){
            throw { statusCode: 404, msg: `Not Found: Posting can not be found`}
        }
        return posting
    }
